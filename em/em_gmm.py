...
class GMM(object):
    ...
    def params_initialize(self, weights, means, covars) -> None:
        """
        ...
        if covars is not None:
            ...
            self.covars = np.empty([self.k, self.feature_size, self.feature_size])
            for k_idx in range(self.k):
                self.covars[k_idx] = np.eye(self.feature_size) * np.random.rand(1) * self.k

    def GMM_EM_single(self):
        """
        利用EM算法优化GMM参数(一次迭代)
        :return: 返回Q函数的值
        """
        ...
        density = np.empty([self.n, self.k])  # 所有self.k个模型对于数据的概率密度值
        ...
        Q_function = weights_hat * np.log(self.weights) + (self.posterior * np.log(density)).sum(axis=0)
        Q_function = Q_function.sum()

        # M步
        means_hat = np.tensordot(self.posterior, self.data, axes=[0, 0])
        # 计算协方差
        covars_hat = np.empty(self.covars.shape)  # shape = (self.k, self.feature_size, self.feature_size)
        for k_idx in range(self.k):
    ...
    def GMM_EM_n(self):
        ...
        EM算法n次迭代
        """
        Q_function_pre = 0 # Q函数初始值
        Q_function_now = 1 # Q函数当前值
        iter = 0
        while abs(Q_function_now - Q_function_pre) > self.tol or iter <= self.max_iter:
            Q_function_pre = Q_function_now
            Q_function_now = self.GMM_EM_single()
            iter += 1
        print(iter)
